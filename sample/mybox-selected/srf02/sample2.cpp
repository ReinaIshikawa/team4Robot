...
#include "/usr/include/linux/i2c-dev.h"
 ...
 main(void) {
  ...
  if (ioctl(fd1, I2C_SLAVE, (0xE0 >> 1)) <
      0) {  // 0xE0(0x11100000) >> 1 = 0x70(0x01110000)
    fprintf(stderr, "Error on slave address 0xE0\n");
    exit(1);
  }
  if (ioctl(fd2, I2C_SLAVE, (0xE2 >> 1)) <
      0) {  // 0xE0(0x11100000) >> 1 = 0x70(0x01110000)
  ...
  // 1つめのセンサに対して
  {  // read from 0xE0
    // コマンドレジスタ0に 0x51:Real Ranging Mode - Result in centimeters
    // を送ることによって測距が始まる
    buf[0] = 0x00;
    buf[1] = 0x51;
    ...
    if ((read(fd1, buf, 1)) != 1) {
      fprintf(stderr, "0xE0 Error on read the Range Low Byte\n");
      exit(1);
    }
  ...
  {  // read from 0xE2
    ...
    if ((write(fd2, buf, 2)) != 2) {
      fprintf(stderr, "0xE2 Error send the read command\n");
      exit(1);
    }
    // Wait for the measurement
    usleep(66000);
    ...
    if ((write(fd2, buf, 1)) != 1) {
      fprintf(stderr, "0xE2 Error on select the Range High Byte\n");
      exit(1);
    }
